...
function ENT:PlayJackpot()
	...
	timer.Simple(45.09, function()
		...
		if self.user ~= nil then
			...
			if character then
				character:GiveMoney(self:GetJackpot()) -- Helix
			end
...
function ENT:Use(act, call)
	if call:IsPlayer() and self.canSpin == true then
		...
		if self:GetMultiplier() == 0 then
			--Take money
			local character = self.user:GetCharacter()
			local canAfford = character:HasMoney(ix.config.Get("doubleOrNothingBet"))
			if canAfford then
				character:TakeMoney(ix.config.Get("doubleOrNothingBet"))
				local jackpotAdd = ix.config.Get("doubleOrNothingJackpotAdd")
				local jackpotAmount = ix.config.Get("doubleOrNothingBet") * (jackpotAdd / 100)
				self:SetJackpot(self:GetJackpot() + jackpotAmount)
			else
				call:ChatPrint(L("dobNotEnoughMoney", self.user) .. " ("..ix.config.Get("doubleOrNothingBet").."€)")
				return false
			end 

			--Set up game
			self:AddMultiplier(2)
			self:EmitSound("doubleornothing/x1.mp3", 65 * 1, 100, 1)
		...
		if shouldSpin and self.userState == true then
			...
			if self:GetMultiplier() > 0 then
				...
				local character = self.user:GetCharacter()
				if character then
					character:GiveMoney(self:GetCashOutAmount()) -- Helix
				end

				self:EmitSound("doubleornothing/cashout.mp3", 65 * 1, 100, 1)
		...
		if shouldSpin and math.random(0, 100) <= ix.config.Get("doubleOrNothingChance") then
			...
			--Play the sound
			self:EmitSound("doubleornothing/x"..self:GetMultiplier()..".mp3", 65 * 1, 100, 1)
			...
			self:EmitSound("doubleornothing/nothing.mp3", 65 * 1, 100, 1)
